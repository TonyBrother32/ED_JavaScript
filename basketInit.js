	...
	htmlGenerateBasket(){
		...
		if(!this.checkEmptyBasket()){
			...
			for (items in this.basket){
				result.push(`<div class='item-wrap' data-name='${this.basket[items].name}'><div class='item-desc'>${this.basket[items].name}</div><div class='item-stats'><div class='item-plus' onclick='db.htmlPlusItem("${this.basket[items].name}")'><img src="plus.png" /></div><div class='item-count' data-name='${this.basket[items].name}'>${this.basket[items].count}</div><div class='item-minus' onclick='db.htmlMinusItem("${this.basket[items].name}")'><img src="minus.png" /></div><div class='items-sum' data-name='${this.basket[items].name}'>${this.calculateItemsCost(items)}rub</div></div></div>`);			
			}
		// если корзина пустая:
		} else {
		...
		for (let i=0; i<nextButtonsLink.length;i++){
			nextButtonsLink[i].addEventListener('click',(e)=>{
				...
				if(i === nextButtonsLink.length-1){
					...
					const arrLinkParent = buttonLink.parentElement.parentElement.parentElement.previousElementSibling.children[1].children[0]; 
					const arrLinkParentSibling = buttonLink.parentElement.parentElement.parentElement.parentElement.nextElementSibling.children[0].children[1].children[0] 
